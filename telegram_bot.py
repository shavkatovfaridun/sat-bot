"""
SAT SAMARKAND — AI-POWERED LEAD BOT (TELEGRAM)
================================================
Uses Claude API to understand and respond to any question in Uzbek/Russian/English.
Logs every conversation to Google Sheets. Escalates hot leads to Faridun instantly.

DEPLOY: Render.com, Railway.app, or any Python host.
RUNTIME: Python 3.10+
"""
...
FARIDUN_CHAT_ID    = int(os.environ["FARIDUN_CHAT_ID"])         # your personal Telegram chat_id
GOOGLE_SHEET_ID    = os.environ["GOOGLE_SHEET_ID"]              # ID of your leads sheet
GOOGLE_CREDS_JSON  = os.environ["GOOGLE_CREDS_JSON"]            # service account JSON (as string)
...
# Load system prompt + knowledge base from files shipped alongside this script
...
# In-memory conversation history (resets on restart — for production, use Redis/DB)
# Map: telegram_user_id -> list of {role, content} messages
...
def log_to_sheet(row: list) -> None:
    """Append a row to the leads Google Sheet."""
    try:
...
async def notify_faridun(context: ContextTypes.DEFAULT_TYPE, lead_info: str) -> None:
    """Send escalation alert to Faridun's personal Telegram."""
    try:
...
def ask_claude(user_id: int, user_message: str) -> tuple[str, bool]:
    """
    Send conversation to Claude API, return (reply_text, is_hot_lead).
    """
    history = conversations.setdefault(user_id, [])
    history.append({"role": "user", "content": user_message})

    # Keep only last 20 turns to manage token usage
    trimmed = history[-20:]
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Admin command: reset conversation memory."""
...
def main() -> None:
    ...
    log.info("SAT Samarkand bot is running...")
